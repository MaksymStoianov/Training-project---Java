package homeworks.homework_21.Task3;
...
public class Autobus {
  ...
  public boolean takePassenger(Passenger passenger) {
    ...
    if (countPassenger < capacity) {
      ...
      if (isPassengerInBus(passenger)) {
        System.out.printf(
            "Пассажир id %d уже в автобусе с id %d\n",
      ...
      countPassenger++;
      System.out.printf(
          "Пассажир id %d завершил посадку в автобус с id %d\n",
          passenger.getId(), this.id
      );
      return true;
    }
  ...
  /**
   * Высаживает пассажира из автобуса.
   *
   * @param passenger Пассажир.
   * @return {@code true}, если пассажир был успешно высажен; иначе, {@code false}.
   */
  public boolean dropPassenger(Passenger passenger) {
    ...
    for (int i = 0; i < this.countPassenger; i++) {
      if (this.passengers[i].getId() != passenger.getId()) continue;

      for (int j = i; j < this.countPassenger - 1; j++) {
        this.passengers[j] = this.passengers[j + 1];
      }

      this.passengers[this.countPassenger - 1] = null;
      this.countPassenger--;
   ...
   *
   * @return Данные о {@code Autobus}.
   */
  public String toString() {
    StringBuilder result = new StringBuilder();

    result.append("Autobus: {")
        .append("id: ").append(this.id)
        .append("; capacity: ").append(this.capacity)
        .append("; driver: ").append(this.driver.toString())
        .append("; autopilot: ").append(this.autopilot.toString())
        .append(" }");
